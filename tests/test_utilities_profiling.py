...
def test_measure_scope(clock):
    with clock.measure():
        time.sleep(0.01)

    assert clock.mean_measures == approx(0.01, abs=0.001)


def test_measure_multiple_scopes_and_provide_mean(clock):
    with clock.measure():
        time.sleep(0.01)
    with clock.measure():
        time.sleep(0.03)

    assert clock.mean_measures == approx(0.02, abs=0.001)


def test_drop_first(clock):
    with clock.measure():
        time.sleep(0.01)
    with clock.measure():
        time.sleep(0.03)

    assert clock.drop_first().mean_measures == approx(0.03, abs=0.001)


def test_total_measures(clock):
    with clock.measure():
        time.sleep(0.01)
    with clock.measure():
        time.sleep(0.03)

    assert clock.total_measures == approx(0.04, abs=0.001)
