...
def test_performance_clock_decorators(caplog, set_logging_active, slow_factor):
...
def test_performance_clock_on_class_methods(caplog, set_logging_active, slow_factor):
    class SomeClass:
        @clock_performance("process")
...
def test_ignore_performance_logging_if_environment_variable_is_not_set(caplog, slow_factor):
    @clock_performance("process")
    def some_process():
        time.sleep(0.01 * slow_factor)
